...
if __name__ == '__main__':
    ...
    # units = []
    # accuracy = []
    # while unit <= max_hidden_layer_units:
    #     accuracy.append(test_digits_single_hidden_layer(unit))
    #     units.append(unit)
    #     if (unit / 10 > 0):
    #         unit += unit / 10
    #     else:
    #         unit += 1
    # plt_units = np.asarray(units)
    # plt_accuracy = np.asarray(accuracy)
    ...
    for i in range(len(units_2)):
        for j in range(len(units_2)):
            accuracy_mat[i][j] = test_digits_two_hidden_layers(units_2[i], units_2[j])
    ...
    plt.pcolor(plt_units_1, plt_units_2, accuracy_mat, cmap='gnuplot')
